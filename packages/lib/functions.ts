declare function GetPlayerIdentifier(
	playerSrc: number,
	identifier: number,
): string;
...
export const distanceVector2 = (
	v1: Vector2Format | [number, number],
	v2: Vector2Format | [number, number],
): number => {
	if (!Array.isArray(v1)) {
		v1 = [v1.x, v1.y];
	}
...
export const lerp = (min: number, max: number, percent: number): number =>
	min * (1 - percent) + max * percent;
