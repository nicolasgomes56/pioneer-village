	...
	serverNamespace.on("connection", (socket) => {
		...
		socket.on("character-update.last-position", async (serverId, coords) => {
			const selectedCharacter =
				Characters.getActiveCharacterForServerId(serverId);
		...
		socket.on("character-event.disconnected", (serverId) => {
			...
			logInfoS(
				`[Characters] character ${serverId} was just disconnected and performed character cleanup`,
			);
		});
	...
	userNamespace.on("connection", (socket) => {
		...
		socket.on("getCharacters", async (cb) => {
			try {
				...
				const prismaCharacters = await Characters.getCharacters(
					socket.data.user.userId,
				);

				for (const character of prismaCharacters) {
					const clothingInventory = await Inventories.getInventoryForUI(
						`clothing:${character.id}`,
					);
		...
		socket.on("character-select.choose", async (characterId, steamId) => {
			const ownsCharacter = await Characters.doesPlayerOwnCharacter(
				characterId,
				steamId,
			);
			if (!ownsCharacter) {
		...
		socket.on("character-update.food-drink", async (food, drink) => {
			...
			await Characters.updateLocalCharacterAtributeWithCharId(
		...
		socket.on(
			...
			(
				...
				Characters.updateLocalCharacterAtributeWithCharId(
