...
export default (prisma: PrismaClient) => {
	...
	const sendMoveOrFailData = (
		...
		if ("requestId" in event) {
			...
			socket.emit("inventorySuccess", {
				identifier: event.identifier,
				requestId,
				requestType,
			});
			userNamespace
				.to(`inventory:${event.identifier}`)
				.emit("inventoryMove", event);
		}
	};
	...
	serverNamespace.on("connection", (socket) => {
		...
		socket.on(
			...
			async (
				...
				try {
					const itemAddEvent = await Inventories.addItem(
						inventoryId,
						itemIdentifier,
						amount,
						metadata,
					...
					if (itemAddEvent) {
						userNamespace
							.to(`inventory:${itemAddEvent.identifier}`)
							.emit("inventoryAdd", itemAddEvent);
					}
					if (itemAddEvent) {
						cb(true);
					} else {
	...
	userNamespace.on("connection", (socket) => {
		...
		socket.on("inventorySubscribe", async (identifier) => {
			...
			userNamespace
				.to(`inventory:${identifier}`)
				.emit("inventoryLoad", inventory);
		});
		...
		socket.on(
			"inventoryStack",
			async (requestId, oldIdentifier, oldSlot, newIdentifier, newSlot) => {
				logInfoC(
					"inventoryStack",
					oldIdentifier,
					oldSlot,
		...
		socket.on("inventory.item-wear", async (itemId: number) => {
			...
			const wearAmount = -1;
			const { success, inventoryIdentifier } =
				await Inventories.changeDurability(itemId, wearAmount);
			logInfoC("success", success, itemId, inventoryIdentifier);

			if (success && inventoryIdentifier) {
				userNamespace
					.to(`inventory:${inventoryIdentifier}`)
					.emit("inventoryWear", itemId, wearAmount);
			}
		});
	});
};
