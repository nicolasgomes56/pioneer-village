...
type PrismaInventoryWithContainerAndItems = PrismaInventory & {
	container: PrismaContainer & { Item: PrismaItem[] };
};
...
const tenDollars = new Array(10).fill({
	identifier: "PV_DOLLAR".GetHashKey(),
	slot: 0,
});
...
class Inventories {
	...
	constructor() {
		if (Inventories.instance) {
			throw new Error(
				"Error: Instantiation failed: Use Inventories.Instance instead of new.",
			);
		}
	}
			...
			await this.prisma.inventory.create({
				data: {
					...
					container: {
						create: {
							Item: {
								create: identifier.startsWith("character:")
									? startingInventory
									: [],
							},
						},
	...
	async getInventory(
		identifier: string,
	): Promise<PrismaInventoryWithContainerAndItems | null> {
		try {
	...
	async getInventoryForUI(
		identifier: string,
	): Promise<UI.Inventory.LoadData | undefined> {
	...
	findSlotForItem(
		inventory: PrismaInventoryWithContainerAndItems,
		itemIdentifier: number,
	) {
		const inventoryType = this.getInventoryType(inventory.identifier);

		const slots: Array<boolean | number> = new Array(inventoryType.slots).fill(
			false,
		);
		...
		for (const s in slots) {
			...
			if (
				slots[slot] === itemIdentifier &&
				slotCounts[slot] < items[itemIdentifier].stackSize
			) {
				return slot;
			}
		}
	...
	isAllowedInInventory(identifier: string, item: Inventory.Item): boolean {
		...
		let isAllowed = false;
		if (
			inventoryType.restrictions === 0 ||
			inventoryType.restrictions & item.restriction
		) {
			isAllowed = true;
		}
	...
	async changeDurability(
		itemId: number,
		durability = -1,
	): Promise<{ success: boolean; inventoryIdentifier?: string }> {
	...
	async stackItem(
		...
		[UI.Inventory.MoveOrFailData, UI.Inventory.MoveOrFailData | null] | void
	> {
		try {
			...
			const oldItems = oldInventory.container.Item.filter(
				(item) => item.slot === oldSlot,
			);
			const oldItemIds = oldItems.map((item) => item.id);

			const newItems = newInventory.container.Item.filter(
				(item) => item.slot === newSlot,
			);
				...
				const eventNew: UI.Inventory.MoveData = {
					...
					items: {
						[newSlot]: {
							...
							metadatas: [
								...newItems.map((item) => item.metadata),
								...oldItems.map((item) => item.metadata),
							],
							durabilities: [
								...newItems.map((item) => item.durability),
								...oldItems.map((item) => item.durability),
							],
				...
				const eventNew: UI.Inventory.MoveData = {
					...
					items: {
						[newSlot]: {
							...
							metadatas: [
								...newItems.map((item) => item.metadata),
								...oldItemsMoved.map((item) => item.metadata),
							],
							durabilities: [
								...newItems.map((item) => item.durability),
	...
	> {
		try {
			...
			if (oldItems.length > 0) {
				if (
					!this.isAllowedInInventory(
						newIdentifier,
						items[oldItems[0].identifier],
					)
				) {
					isAllowed = false;
				}
			...
			for (const item of inventory.container.Item) {
				if (item.identifier === "PV_DOOR_KEY".GetHashKey()) {
					if (
						...
						if (
							metadata?.doorHashes &&
							(metadata?.doorHashes as Prisma.JsonArray).includes(doorHash)
						) {
							return true;
						}
