import type {
	Prisma,
	Characters as PrismaCharacter,
	PrismaClient,
	Faces as PrismaFace,
} from "@prisma/client";
import type { Socket } from "socket.io";
import type { JSONObject } from "three/examples/jsm/loaders/IFCLoader";
import { logInfoC, logInfoS } from "../helpers/log";

type PrismaCharacterWithFace = PrismaCharacter & { face: PrismaFace | null };

export const GetFaceDataFromDatabase = (
	result: PrismaCharacterWithFace,
): Game.Face => {
...
class Characters {
	...
	constructor() {
		if (Characters.instance) {
			throw new Error(
				"Error: Instantiation failed: Use Characters.Instance instead of new.",
			);
		}
		this.startIntervals();
	}
	...
	updateLocalCharacterAtributeWithCharId(
		charId: number,
		attribute: keyof CharacterData,
		newValue: any,
	) {
	...
	async createCharacter(
		...
		const character = await this.prisma.characters.create({
			data: { ...characterData, accountId: ownerId },
		});
		await this.prisma.faces.create({
			data: { ...faceData, characterId: character.id },
		});
	...
	async setLastCoords(characterId: number, coords: Vector3Format) {
		...
		this.updateLocalCharacterAtributeWithCharId(
			characterId,
			"lastX",
			coords.x.toFixed(3),
		);
	...
	private async updateCharacterFoodAndDrink(
		characterId: number,
		food: number,
		drink: number,
	) {
	...
	async updateCharacterHealthMetadata(
		characterId: number,
		metadata: CharacterHealthMetadata,
	) {
	...
	async getCharacterHealthMetadata(
		characterId: number,
	): Promise<CharacterHealthMetadata | undefined> {
		const result = await this.prisma.characters.findFirst({
			where: {
				id: characterId,
			},
		...
		if (result && result.healthMetadata) {
			const finalData = result.healthMetadata as Prisma.JsonObject;
	...
	private async updateDatabaseWithPlayerMetadata(character: CharacterData) {
		...
		await this.updateCharacterHealthMetadata(
			character.id,
			character.healthMetadata,
		);
		await this.updateCharacterFoodAndDrink(
			character.id,
			character.food,
			character.drink,
		);
	...
	async getCharacterCurrencies(
		charId: number,
	): Promise<CharacterCurrencies | undefined> {
		const result = await this.prisma.characters.findFirst({
			where: {
				id: charId,
			},
	...
	async updateCharacterCurrencies(
		characterId: number,
	...
	setCharacterCurrency(
		characterId: number,
		type: keyof CharacterCurrencies,
		amount: number,
	): boolean {
		const character = this.getActiveCharacterForCharacterId(characterId);
		if (!character) {
			logInfoS(
				"Attempted to get character",
				characterId,
				"but is offline aborting",
			);
			return false;
		}
	...
	async setActiveCharacter(
		...
		if (!character) {
			logInfoC(
				"Attempted to get character with id",
				charId,
				"but returned null",
			);
			return;
		...
		socket.emit(
			"character-client-update.getCharacter",
