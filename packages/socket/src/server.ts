...
const requireServerKey = (socket: Socket, next: (err?: Error) => void) =>
	next(
		socket.handshake.auth.token === process.env.SOCKET_SERVER_KEY
			? undefined
			: new Error("invalid"),
	);
...
const logAll = (socket: Socket, next: (err?: Error) => void) => {
	socket.onAny((...args: any[]) =>
		logEvent(socket.nsp.name, inspect(args, { depth: null, colors: true })),
	);
	// socket.onAny((...args: any[]) =>
...
export const serverNamespace: Namespace<
	SocketServer.Server & SocketServer.ServerEvents,
	SocketEvents
> = io.of("/server").use(requireServerKey).use(logAll);

export const userNamespace: Namespace<
	SocketServer.Client & SocketServer.ClientEvents,
	UISocketEvents
> = io.of("/users").use(requireUuid).use(logAll);
