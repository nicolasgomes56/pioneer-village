declare namespace UI {
	type onUI = <T extends keyof UIEvents>(
		evtName: T,
		callback: (...args: Parameters<UIEvents[T]>) => void,
	) => void;
	type onUICall = <T extends keyof UIRPC>(
		evtName: T,
		callback: (
			...args: Parameters<UIRPC[T]>
		) => Promise<ReturnType<UIRPC[T]>> | ReturnType<UIRPC[T]>,
	) => void;
	type emitUI = <T extends keyof UIEvents>(
		evtName: T,
	...
	type awaitUI = <T extends keyof UIRPC>(
		evtName: T,
		...args: Parameters<UIRPC[T]>
	) => Promise<ReturnType<UIRPC[T]>>;
...
declare interface SocketForwardEvents {
	["character-select.choose"]: (
		characterId: number,
		steam: Game.playerSteamId,
	) => void;
}
...
declare interface UIEvents {
	...
	["customization.set-tint-by-category"]: (
		category: string,
		tint: Customization.Palette,
	) => void;
